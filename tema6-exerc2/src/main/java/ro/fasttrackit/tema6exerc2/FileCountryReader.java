...
{
	private static long countryCount = 1;
	...
	private static Country parseCountryString(String countryLine)
	{
		...
		if(countryLine != null)
		{
			...
			if(countryParameters.length == 6)
			{
				...
				try
				{
					Long id = Long.valueOf(countryCount);  // id-ul se genereaza, nu se citeste din fisierul .txt
					countryCount++;
					
					String name = countryParameters[0];
					String capital = countryParameters[1];
					Long population = Long.valueOf(countryParameters[2]);
					Integer area  = Integer.valueOf(countryParameters[3]);
